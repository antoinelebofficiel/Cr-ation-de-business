# MODE LOUP / TDAH — LA SEULE PAGE QUI COMPTE

Lis ça. Ferme le reste. Exécute.

## Cible
**15 clients payants en août** (contrats ou interventions encaissées).  
Ensuite septembre → **12–15k CA**.

## Offre (une phrase, point)
> On entretient vos vitrines / vitres sous contrat, créneaux fixes, sans que vous y pensiez. À partir de 89 €/mois commerces · devis 24h particuliers.

Pas de perfection ops. Pas de recruter une équipe salariée. Pas de branding 3 semaines.

## Règles du loup
1. **Appeler / ads > préparer**
2. **Encaisser > polish**
3. **Tu ne nettoies jamais** — si 0 prestataire dispo = tu reportes le créneau, tu ne prends pas le balai
4. **Contrat > one-shot** — toujours pousser mensuel / trimestriel
5. Envie d’“améliorer le système” → **40 appels ou 50 € ads d’abord**
6. Score < 3 → skip 3 sec
7. Sam / dim comptent (ads + closes)
8. Zone figée = **Lorient Agglomération (25 km)**. Pas Vannes/Quimper avant 15 clients

---

## AUJOURD’HUI

### Chargeur (30–40 min)
- [ ] 40 fiches Google Maps commerces (vitrines) — voir [process-liste.md](01-cible/process-liste.md)
- [ ] Score ≥3 sinon poubelle
- [ ] Tél + nom commerce + type vitrine

### FIRE B2B (40–60 min ou 18h–19h)
- [ ] **30–40 appels** commerces / restos / agences
- [ ] Close = 1er passage sous 7j + contrat mensuel proposé
- [ ] SMS après non-décroché

### FIRE B2C (parallèle)
- [ ] Campagne Meta active (budget min **30 €/jour**)
- [ ] Landing live → formulaire → rappel <15 min
- [ ] Close forfait ou intervention

### DEBRIEF (10 min)
- [ ] appels / RDV / closes / CA encaissé
- [ ] 1 phrase de pitch corrigée
- [ ] Prestataires dispo cette semaine (oui/non)

### Week-end
- **Sam** : 60 appels B2B + ads on
- **Dim** : 30 appels sniper score 5 + closes leads ads

---

## Script appel (T0 commerce)

> Bonjour, je tombe sur [Commerce] — c’est pour l’entretien de vos vitrines.  
> Qui gère ça aujourd’hui, le gérant ou un prestataire ?  
> …  
> On passe en créneau fixe, vitrines nickel avant ouverture, à partir de 89 €/mois.  
> Je vous propose un 1er passage cette semaine — mardi matin ou jeudi matin ?

**Close** : 1er passage + prélèvement / virement contrat.  
**Pas de devis PDF de 4 pages.** Photo WhatsApp + prix oral + SMS récap.

---

## Scoreboard août
| Date | Tu dois |
|---|---|
| 7 août | Structure légale OK + 1er prestataire + 1er client payant |
| 15 août | **5** clients / interventions encaissées |
| 31 août | **15** = MVP validé |

## Après 15
Sept → ads scale + 50 appels/jour B2B → **12–15k CA**.

---

## Interdits
- Nettoyer toi-même “juste cette fois”
- Embaucher CDI avant 10k CA/mois récurrent
- Ouvrir 3 villes
- Refaire le site 4 fois
- Autre business en parallèle Phase 0

## Autorisé
- Prix au feeling si close bloqué (±15 %)
- Prestataire bancal mais assuré
- Landing moche qui convertit
- Micro-entreprise pour démarrer

---

## Ordre d’attaque
1. Choisir **1 agglo** (voir [01-cible/STATUS.md](01-cible/STATUS.md))
2. Trouver **1 prestataire** cette semaine
3. **25 appels** ce soir + Meta 30 €/j
4. Jusqu’à 15 clients

**Vente sans ops = tu livres pas.**  
**Ops sans vente = hobby.**  
**Toi = vendeur + chef d’orchestre. Point.**
