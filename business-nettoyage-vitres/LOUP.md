# MODE LOUP / TDAH — LA SEULE PAGE QUI COMPTE

Lis. Ferme le reste. Exécute.

**Décision :** Clean&Pro = **seul** business — [DECISION-CLEAN-AND-PRO.md](DECISION-CLEAN-AND-PRO.md).  
**Cette semaine :** **[SEMAINE-10X-2000.md](SEMAINE-10X-2000.md)** → **2 000 €**.

## Offre (une phrase)
> On entretient vos vitrines / vitres sous contrat, créneaux fixes, sans que vous y pensiez.  
> Sur mesure commerces, à partir de 89 €/mois · particuliers dès 99 €.

## Règles du loup
1. **Appeler / ads > préparer**
2. **Encaisser > polish**
3. **Tu ne nettoies jamais**
4. **Contrat > one-shot** (upsell dès le 1er passage)
5. Envie d’“améliorer le système” → **40 touches ou 50 € ads d’abord**
6. Score < 3 → skip 3 sec
7. **Samedi = guerre** (80 appels/visites)
8. Zone = **Lorient Agglo 25 km**. Point.

## AUJOURD’HUI (ordre)

### 1. Setup bloquant (si pas fait)
- [ ] Prestataire #1 contacté / confirmé
- [ ] WhatsApp + paiement prêts
- [ ] **2 Instant Forms** : B2B commerces + B2C
- [ ] Meta ON : **~35 €/j B2B + ~15 €/j B2C** — [meta-ads.md](03-demarchage/meta-ads.md)

### 2. Chargeur (20 min)
- [ ] 3 créas B2B (avant/après vitrine) uploadées
- [ ] File portes samedi ≥40 commerces

### 3. FIRE soir (17h30–21h)
- [ ] **100 % leads Meta** — B2B d’abord, puis B2C
- [ ] Closes = date + prix + paiement
- [ ] SMS non-décrochés

### 4. FIRE samedi
- [ ] 80 portes/appels commerces (complément ads)
- [ ] Close passage sous 7j

### 5. Debrief (10 min)
- [ ] `daily-tracker.csv` : leads B2B/B2C · closes · CA · spend

## Script commerce (T0)
> Bonjour, Antoine Bauché — Clean&Pro.  
> Pour vos vitrines : interne ou presta ?  
> Créneau fixe avant ouverture, sur mesure dès 89 €/mois.  
> Mardi 7h30 ou jeudi 7h30 ?

**Close** = 1er passage daté. Script : [03-demarchage/cold-call-script.md](03-demarchage/cold-call-script.md)

## Scoreboard
| Date | Preuve |
|---|---|
| 10 août | **2 000 €** |
| 17 août | 5 payants |
| 31 août | 15 payants (≥5 récurrents) |

## Interdits
- Autre business
- Nettoyer toi-même
- Embaucher CDI
- 3 villes
- Refaire le site

## Autorisé
- **Sur mesure** (jamais &lt; 89 €/mois commerce)
- Prestataire bancal mais assuré
- Landing moche qui convertit
- Micro-entreprise

**Toi = vendeur + chef d’orchestre. Point.**
